...
{
    private readonly ReceivablesViewModel _vm;

    private static readonly string[] AgingColors = { "#10B981", "#F59E0B", "#EF4444", "#8B5CF6" };

    public ReceivablesView() { InitializeComponent(); }

    public ReceivablesView(ReceivablesViewModel vm)
    {
        InitializeComponent();
        _vm = vm;
    ...
    private async Task OpenPickerAndPay()
    {
        ...
        var combo = new ComboBox
        {
            ...
            ItemTemplate = new FuncDataTemplate<ReceivableRow>((row, _) =>
                new TextBlock { Text = $"{row?.CustomerName} — ${row?.Balance:F2} due" })
        };
        ...
        stack.Children.Add(new TextBlock { Text = "Select a customer to receive payment from:", FontSize = 13, Foreground = new SolidColorBrush(Color.Parse("#334155")) });
        ...
        {
            if (combo.SelectedItem is ReceivableRow selectedRow)
            {
                picker.Close();
                var amount = await PaymentDialog.ShowAsync(owner, selectedRow.CustomerName, selectedRow.Balance);
                if (amount.HasValue)
                {
                    await _vm.SubmitPaymentForApprovalAsync(selectedRow, amount.Value);
                    await LoadAndRender();
                }
            }
        };
    ...
    {
        if (e.Source is Button { Name: "PayBtn" } btn && btn.DataContext is ReceivableRow row)
        {
            var owner = TopLevel.GetTopLevel(this) as Window;
            if (owner == null || row.Balance <= 0) return;
            var amount = await PaymentDialog.ShowAsync(owner, row.CustomerName, row.Balance);
            if (amount.HasValue)
            {
    ...
    {
        TotalReceivablesText.Text = $"${_vm.TotalReceivables:F2}";
        TotalReceivablesSubText.Text = $"From {_vm.CustomerCount} Customers";
        TotalCollectedText.Text = $"${_vm.TotalCollectedThisMonth:F2}";
        OutstandingText.Text = $"${_vm.OutstandingAmount:F2}";
        OutstandingSubText.Text = $"From {_vm.AllRows.Count(r => r.Balance > 0)} Customers";
    ...
    private void RenderTopList()
    {
        ...
        foreach (var row in top)
        {
            ...
            header.Children.Add(new TextBlock { Text = row.CustomerName, FontSize = 12, Foreground = new SolidColorBrush(Color.Parse("#334155")) });
    ...
    private void DrawDonut()
    {
        ...
        for (int i = 0; i < _vm.AgingBuckets.Count; i++)
        {
            ...
            DrawArcSegment(canvas, cx, cy, radius, thickness, cursor * 360, (cursor + frac) * 360, Color.Parse(AgingColors[i]));
            cursor += frac;
            AddDonutLegendItem(canvas, w - 120, legendY + i * 24, AgingColors[i], bucket.Label, $"{bucket.Amount:F0} ({frac:P0})");
        }
    }
