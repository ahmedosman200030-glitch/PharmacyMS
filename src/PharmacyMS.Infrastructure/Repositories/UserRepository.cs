...
public class UserRepository : IUserRepository
{
    ...
    {
        using var conn = _context.CreateConnection();
        return await conn.QueryAsync<User>("SELECT * FROM Users ORDER BY FullName");
    }
    ...
    public async Task<int> CreateAsync(User user)
    {
        ...
        return await conn.ExecuteScalarAsync<int>($@"
            ...
            VALUES (@Username, @PasswordHash, @FullName, @Role, @IsActive, @Permissions, {_context.NowExpr()})
            {_context.InsertIdSuffix()};", user);
    }
    ...
    {
        using var conn = _context.CreateConnection();
        await conn.ExecuteAsync("UPDATE Users SET IsActive=1 WHERE Id=@Id", new { Id = id });
    }

    public async Task UpdateLastLoginAsync(int userId)
    {
        using var conn = _context.CreateConnection();
        await conn.ExecuteAsync($"UPDATE Users SET LastLogin={_context.NowExpr()} WHERE Id=@Id", new { Id = userId });
    }
}
